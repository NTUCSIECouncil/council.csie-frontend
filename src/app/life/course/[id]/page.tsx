  ...
  return (
    <div>
      <div className="flex flex-col items-start gap-2 py-4">
        <LifeCourseTopic
          topic={courseData.topic.topic}
          subtopic={courseData.topic.subtopic ?? ''}
          lecturer={courseData.topic.lecturer ?? ''}
          author={courseData.topic.author}
        />
          ...
          <div key={subtopic.topicName} className="w-full">
            ...
            {subtopic.blocks.map((block, index) => {
              switch (block.layout) {
                ...
                case 'table':
                  return (
                    <Table key={index} table={block.content as string[][]} />
                  );
                case 'tldr':
                ...
                case 'links':
                  return (
                    <div
                      key={index}
                      className="grid xl:grid-cols-2 grid-cols-1 gap-3 w-full"
                    >
                      {(block.content as string[][]).map(
                        (website: string[], index: number) => (
                          <Link href={website[0]} key={index}>
