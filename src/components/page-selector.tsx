...
const StyledLink = ({
  ...
  return (
    <Link
      href={href}
      className="font-bold flex justify-center items-center w-12 h-12 
...
const PageNum = ({
  ...
  if (newindex + 1 > 0 && newindex * limit < total) {
    if (index_off === 0)
      ...
      return (
        <StyledLink href={`?${baseParams}index=${newindex.toString()}`}>
          {newindex + 1}
        </StyledLink>
      );
  } else
...
const PageSelector = ({
  ...
  return (
    <div className="flex justify-between w-full mt-4 mb-10 h-15 relative z-10 max-w-200">
      ...
      <StyledLink
        href={`?${baseParams}index=${Math.max(0, index - 1).toString()}`}
      >
        <FaAngleLeft />
      </StyledLink>
      {pageIndexOffset.map(num => (
        <PageNum
          key={num}
      ...
      <StyledLink
        href={`?${baseParams}index=${Math.min(index + 1, Math.floor(Math.max(total - 1, 0) / limit)).toString()}`}
      >
        <FaAngleRight />
      </StyledLink>
      <StyledLink
        href={`?${baseParams}index=${Math.floor(Math.max(total - 1, 0) / limit).toString()}`}
      >
        <FaAngleDoubleRight />
      </StyledLink>
    </div>
